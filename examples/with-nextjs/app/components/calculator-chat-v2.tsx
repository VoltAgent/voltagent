...
export function CalculatorChatV2() {
  ...
  const [initialMessages, setInitialMessages] = useState<UIMessage[]>([
    {
      ...
      parts: [
        {
          type: "text",
          text: "Hello! I'm your AI-powered calculator (V2 - Direct AI SDK). You can write your calculations in natural language. For example: '5 plus 3 times 2' or '(25 + 75) / 4'.",
        },
      ],
    },
  ]);

  // Load messages from storage when userId or conversationId changes
  useEffect(() => {
    const loadMessages = async () => {
      try {
        const response = await fetch(
          `/api/messages?conversationId=${conversationId}&userId=${userId}`,
        );
        if (response.ok) {
  ...
  console.log("Initial messages V2:", initialMessages);

  const { messages, sendMessage, status, error } = useChat({
    transport: new DefaultChatTransport({
      api: "/api/chat-v2", // Use the new V2 endpoint
      body: () => ({
        userId,
        conversationId,
      }),
  ...
  return (
    <div className="bg-[#1b1b1b] border-2 border-[#333333] rounded-xl shadow-xl overflow-hidden max-w-2xl mx-auto">
      <div className="bg-[#333333] p-6">
        <h2 className="text-white text-2xl font-bold">AI Calculator Chat V2</h2>
        <p className="text-gray-400 text-sm mt-1">
          Direct AI SDK Integration - No Provider Abstraction
        </p>

        {/* User and Conversation Settings */}
        <div className="mt-4 flex gap-4">
          <div className="flex-1">
            ...
            <input
              ...
              className="w-full px-3 py-1 bg-[#1b1b1b] text-white border border-gray-600 rounded text-sm focus:outline-none focus:border-[#24f2ff]"
              placeholder="Enter user ID"
            />
          </div>
          <div className="flex-1">
            <label className="text-xs text-gray-400 block mb-1">Conversation ID</label>
            <input
              type="text"
                ...
                <div className="text-sm">
                  {message.role === "assistant" && (
                    <div className="flex items-center mb-1">
                      ...
                      <span className="text-xs text-gray-400">AI Assistant V2</span>
                    </div>
                  )}
              ...
              <div className="flex justify-start">
                <div className="bg-gray-800 text-gray-300 border border-gray-700 max-w-xs lg:max-w-md px-4 py-2 rounded-lg">
                  <div className="flex items-center">
                    <div className="w-2 h-2 bg-[#24f2ff] rounded-full mr-2" />
                    <span className="text-xs text-gray-400 mr-2">AI Assistant V2</span>
      ...
      <div className="border-t border-gray-700 p-4">
        ...
        <div className="mt-2 text-xs text-gray-500">
          ...
          <p className="text-blue-400">💡 Direct AI SDK Integration - No Provider Layer!</p>
        </div>
      </div>
    </div>
  );
