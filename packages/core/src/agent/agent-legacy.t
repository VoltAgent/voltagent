...
import {
  type StreamEvent,
  // streamEventForwarder, // Removed - SubAgent support removed
...
// SubAgent support removed - use AgentV2 instead
...
import type {
  ...
  StreamTextResponse,
  // SupervisorConfig, // Removed - SubAgent support removed
...
/**
 * Legacy Agent class for interacting with AI models
 * @deprecated Use Agent instead
 */
export class AgentLegacy<TProvider extends { llm: LLMProvider<unknown> }> {
  /**
  ...
  /**
   * Sub-agent manager for the agent - DEPRECATED: Use AgentV2 instead
   */
   ...
   */
  // private readonly supervisorConfig?: SupervisorConfig; // Removed - SubAgent support removed
  ...
  constructor(
    options: AgentOptions &
      TProvider & {
        ...
        // subAgents?: SubAgentConfig[]; // DEPRECATED: Use AgentV2 instead
    ...
    // this.supervisorConfig = options.supervisorConfig; // Removed - SubAgent support removed
  ...
  protected async getSystemMessage({
    ...
    // SubAgent support removed - use AgentV2 instead
    // if (false) { // this.subAgentManager.hasSubAgents()
   ...
   */
  // SubAgent support removed - use AgentV2 instead
  // private async prepareAgentsMemory(_contextMessages: BaseMessage[]): Promise<string> {
  //   try {
  //     // SubAgent support removed - use AgentV2 instead
  //     const subAgents: any[] = []; // this.subAgentManager.getSubAgents();
  ...
  private calculateMaxSteps(): number {
    // SubAgent support removed - use AgentV2 instead
    return this.maxSteps || 10; // this.subAgentManager.calculateMaxSteps(this.maxSteps);
  }
  ...
  private async prepareTextOptions(
    ...
    const {
      ...
      historyEntryId,
      operationContext,
      internalStreamForwarder: _internalStreamForwarder,
      logger,
    } = options;
    ...
    //   // SubAgent support removed - use AgentV2 instead
    //   const delegateTool = null as any; // this.subAgentManager.createDelegateTool({
    //   //   sourceAgent: this,
    ...
    signal.addEventListener("abort", async () => {
      ...
      const agentCancelledEvent = {
        ...
        metadata: {
          ...
          id: this.id,
          context: Object.fromEntries(operationContext.userContext.entries()) as Record<
            string,
            unknown
          >,
        },
  ...
  public async generateText(
    ...
    const { messages: contextMessages, conversationId: finalConversationId } =
      await this.memoryManager.prepareConversationContext(
        operationContext,
        input as any, // Temporary cast - this Agent class is deprecated
        userId,
    ...
    try {
      ...
      const systemMessageResponse = await this.getSystemMessage({
        ...
        contextMessages: contextMessages as any, // Type cast - deprecated Agent class
        operationContext,
      });
      ...
      const systemMessages = Array.isArray(systemMessageResponse.systemMessages)
        ...
        : [systemMessageResponse.systemMessages];
      messages = [...systemMessages, ...(contextMessages as any)];
      ...
      try {
        const prepareResult = await this.getMergedHooks(internalOptions).onPrepareMessages?.({
          messages: [...(messages as any)], // Pass a copy to prevent direct mutation - deprecated Agent class
          agent: this,
          context: operationContext,
        });
        ...
        if (prepareResult?.messages && Array.isArray(prepareResult.messages)) {
          messages = prepareResult.messages as any; // Type cast - deprecated Agent class
        }
      } catch (error) {
      ...
      methodLogger.debug("[LLM] - Generating text", {
        messages: messages.map((msg) => ({
          role: msg.role,
          content: (msg as any).content, // Type cast - deprecated Agent class
        })),
        maxSteps,
  ...
  public async streamText(
    ...
    const systemMessages = Array.isArray(systemMessageResponse.systemMessages)
      ? systemMessageResponse.systemMessages
      : [systemMessageResponse.systemMessages];
    let messages = [...systemMessages, ...(contextMessages as any)]; // Type cast - deprecated Agent class
    messages = await this.formatInputMessages(messages as any, input);
    ...
    const agentStartEvent: AgentStartEvent = {
      ...
      metadata: {
        ...
        >,
        systemPrompt: systemMessages,
        messages: messages as any, // Type cast - deprecated Agent class
        promptMetadata: systemMessageResponse.promptMetadata,
        isDynamicInstructions: systemMessageResponse.isDynamicInstructions,
        modelParameters: {
          model: this.getModelName(),
    ...
    const internalStreamEventForwarder = async (event: StreamEvent) => {
      ...
      if (event.subAgentId && !subAgentStatus.has(event.subAgentId)) {
   ...
   */
  public addSubAgent(_agentConfig: any): void {
    throw new Error(
      "SubAgent support has been removed from Agent class. Please use AgentV2 instead.",
    );
  }
   ...
   * Remove a sub-agent
   */
  public removeSubAgent(_agentId: string): void {
    throw new Error(
      "SubAgent support has been removed from Agent class. Please use AgentV2 instead.",
    );
  }
   ...
   * Get all sub-agents
   */
  public getSubAgents(): any[] {
    return []; // SubAgent support removed - use AgentV2 instead
  }
