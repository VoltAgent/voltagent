...
```bash
npm install @voltagent/fastify fastify
# or
yarn add @voltagent/fastify fastify
# or
pnpm add @voltagent/fastify fastify
```
...
import fastifyVoltAgent from "@voltagent/fastify";

const app = fastify();

await app.register(fastifyVoltAgent, {
  agents: {
    myAgent,
  },
  telemetryExporter: {
...
// access the agents via the fastify instance
app.get("/agents", async (request, reply) => {
  const agents = app.volt.getAgents();
  return agents;
});
```
