...
export class LibSQLObservabilityAdapter implements ObservabilityStorageAdapter {
  ...
  constructor(options: LibSQLObservabilityOptions = {}) {
    ...
    this.evalScoreTable = `${this.tablePrefix}_eval_scores`;
  ...
  private async initializeDatabase(): Promise<void> {
    try {
      ...
      await this.client.execute(`
        CREATE TABLE IF NOT EXISTS ${this.evalScoreTable} (
          id TEXT PRIMARY KEY,
      ...
      await this.client.execute(`
        CREATE INDEX IF NOT EXISTS idx_${this.tablePrefix}_eval_scores_trace_id
        ON ${this.evalScoreTable}(trace_id)
      `);

      await this.client.execute(`
        CREATE INDEX IF NOT EXISTS idx_${this.tablePrefix}_eval_scores_agent_id
        ON ${this.evalScoreTable}(agent_id)
      `);

      await this.client.execute(`
        CREATE INDEX IF NOT EXISTS idx_${this.tablePrefix}_eval_scores_run_id
        ON ${this.evalScoreTable}(run_id)
      `);

      await this.client.execute(`
        CREATE INDEX IF NOT EXISTS idx_${this.tablePrefix}_eval_scores_dataset_id
        ON ${this.evalScoreTable}(dataset_id)
      `);

      await this.client.execute(`
        CREATE INDEX IF NOT EXISTS idx_${this.tablePrefix}_eval_scores_scorer_id
        ON ${this.evalScoreTable}(scorer_id)
      `);

      await this.client.execute(`
        CREATE INDEX IF NOT EXISTS idx_${this.tablePrefix}_eval_scores_created_at
        ON ${this.evalScoreTable}(created_at DESC)
      `);
  ...
  async saveEvalScore(record: ObservabilityEvalScoreRecord): Promise<void> {
    ...
    try {
      const metadataJson = record.metadata ? safeStringify(record.metadata) : null;
      const samplingJson = record.sampling ? safeStringify(record.sampling) : null;
      const updatedAt = record.updatedAt ?? record.createdAt;

      await this.client.execute({
        sql: `
          INSERT INTO ${this.evalScoreTable} (
            id,
            agent_id,
            agent_name,
            trace_id,
          ...
          ON CONFLICT(id) DO UPDATE SET
            ...
            dataset_item_hash = excluded.dataset_item_hash,
        ...
        args: [
          ...
          record.datasetItemHash ?? null,
          record.datasetItemId ?? null,
          record.scorerId,
          record.scorerName ?? null,
          record.status,
          record.score ?? null,
          record.threshold ?? null,
          typeof record.thresholdPassed === "boolean" ? (record.thresholdPassed ? 1 : 0) : null,
          metadataJson,
          samplingJson,
      ...
      this.logger.error("Failed to save eval score", { error, recordId: record.id });
      throw error;
    }
  }
  ...
  async listEvalScores(
    ...
    if (filter.datasetVersionId) {
      conditions.push("dataset_version_id = ?");
      args.push(filter.datasetVersionId);
    }

    if (filter.datasetItemId) {
      conditions.push("dataset_item_id = ?");
      args.push(filter.datasetItemId);
    }

    if (typeof filter.thresholdPassed === "boolean") {
      conditions.push("threshold_passed = ?");
      args.push(filter.thresholdPassed ? 1 : 0);
    }
    ...
    const offsetClause = filter.offset !== undefined ? "OFFSET ?" : "";
    ...
    try {
      ...
      return result.rows.map((row) => this.mapEvalScoreRow(row));
    } catch (error) {
      this.logger.error("Failed to list eval scores", { error, filter });
      throw error;
    }
  }

  private mapEvalScoreRow(row: any): ObservabilityEvalScoreRecord {
    ...
    if (row.sampling && typeof row.sampling === "string") {
      try {
    ...
    return {
      ...
      datasetItemHash: row.dataset_item_hash ?? undefined,
      datasetItemId: row.dataset_item_id ?? undefined,
      scorerId: row.scorer_id as string,
      scorerName: row.scorer_name ?? undefined,
      status: row.status as ObservabilityEvalScoreRecord["status"],
      score: row.score !== null && row.score !== undefined ? Number(row.score) : null,
      threshold:
        row.threshold !== null && row.threshold !== undefined ? Number(row.threshold) : null,
      thresholdPassed:
        row.threshold_passed !== null && row.threshold_passed !== undefined
          ? Boolean(row.threshold_passed)
          : null,
      metadata,
      sampling: sampling as ObservabilityEvalScoreRecord["sampling"],
