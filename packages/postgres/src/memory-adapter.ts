...
export class PostgreSQLMemoryAdapter implements StorageAdapter {
  ...
  /**
   * Get the current table prefix being used by the adapter
   */
  ...
  async getTableStats(tableName: string): Promise<{
    ...
    // Get row count
    const countResult = await this.pool.query(`SELECT COUNT(*) as count FROM ${fullTableName}`);
  ...
  /**
   * Get table schema information (columns and their types)
   */
  ...
  /**
   * Drop all managed tables (with the configured table prefix)
  ...
  async dropManagedTables(): Promise<void> {
    ...
    try {
      ...
      await client.query("COMMIT");
      this.log(`Dropped ${tables.length} managed tables`);
    } catch (error) {
      await client.query("ROLLBACK");
      throw error;
    } finally {
  ...
  /**
   * Truncate all managed tables (delete all data but keep structure)
   */
  async truncateManagedTables(): Promise<void> {
    ...
    try {
      ...
      await client.query("COMMIT");
      this.log(`Truncated ${tables.length} managed tables`);
    } catch (error) {
      await client.query("ROLLBACK");
      throw error;
    } finally {
   ...
   * Useful for custom queries or maintenance tasks
   */
  ...
  /**
   * Vacuum analyze all managed tables for better performance
   */
