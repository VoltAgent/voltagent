...
/**
 * Get live evaluation scores from the observability store
 */
export async function getLiveEvalScoresHandler(
  query: Record<string, string>,
  deps: ServerProviderDeps,
): Promise<any> {
  try {
    ...
    if (!storage || typeof storage.listEvalScores !== "function") {
      return {
        success: false,
        error: "Eval score storage not available",
      };
    }
    ...
    if (query.datasetId) {
      filter.datasetId = query.datasetId;
    }
    if (query.datasetVersionId) {
      filter.datasetVersionId = query.datasetVersionId;
    }
    if (query.datasetItemId) {
      filter.datasetItemId = query.datasetItemId;
    }
    if (query.thresholdPassed !== undefined) {
      filter.thresholdPassed = query.thresholdPassed === "true";
    }
    if (query.limit) {
    ...
    const scores = await storage.listEvalScores(filter as any);
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Failed to fetch live evaluation scores",
    };
  }
}
